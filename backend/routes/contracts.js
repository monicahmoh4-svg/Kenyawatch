...
router.get('/search', async (req, res) => {
  try {
    ...
    if (!q || q.trim().length < 2) return res.json({ total: 0, page: 1, limit: 20, results: [] });
    const searchTerm = `%${q.trim()}%`;
    const lim = Math.min(Number(limit) || 20, 100);
    const off = (Math.max(Number(page) || 1, 1) - 1) * lim;
    const countRes = await pool.query(
      `SELECT COUNT(*) FROM contracts WHERE title ILIKE $1 OR supplier ILIKE $1 OR county ILIKE $1 OR contract_id ILIKE $1 OR scope ILIKE $1`,
      [searchTerm]
    );
    const rowsRes = await pool.query(
      `SELECT * FROM contracts WHERE title ILIKE $1 OR supplier ILIKE $1 OR county ILIKE $1 OR contract_id ILIKE $1 OR scope ILIKE $1 ORDER BY risk_score DESC, year DESC LIMIT $2 OFFSET $3`,
      [searchTerm, lim, off]
    );
    res.json({ total: Number(countRes.rows[0].count), page: Number(page) || 1, limit: lim, results: rowsRes.rows });
  } catch (e) { res.status(500).json({ error: 'Failed to search contracts' }); }
});

router.get('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('SELECT * FROM contracts WHERE id = $1 OR contract_id = $1', [id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Contract not found' });
    res.json(result.rows[0]);
  } catch (e) { res.status(500).json({ error: 'Failed to fetch contract' }); }
});
